    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEditing
              ? 'Modificá los datos del libro.'
              : 'Completá los datos para registrar un nuevo libro.'}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <FormField
              label="Stock Actual"
              value={String(form.stock_actual)}
              onChange={(v) => handleChange('stock_actual', parseInt(v) || 0)}
            ...
            <div className="space-y-2">
              ...
              <select
                value={form.estado}
                onChange={(e) =>
                  handleChange('estado', e.target.value as LibroResumen['estado'])
                }
                className="w-full h-10 px-3 rounded-lg bg-secondary ring-1 ring-border focus:ring-2 focus:ring-primary outline-none text-sm"
              >
                {ESTADOS.map((estado) => (
...
const FormField = ({
  ...
  <div className="space-y-2">
    ...
    <input
      ...
      className={`w-full h-10 px-3 rounded-lg bg-secondary ring-1 ${
        error ? 'ring-destructive' : 'ring-border'
      } focus:ring-2 focus:ring-primary outline-none text-sm transition-all`}
